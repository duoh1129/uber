...
export const GetTripDistance = (data) => {
    if (data) {
        const arr = Object.keys(data)
            .filter(function (i) {
                return data[i].status == 'STARTED' || data[i].status == 'ENDED'
            })
            .map(i => {
                data[i].id = i
                return data[i]
        ...
        if (arr.length == 0 || arr.length == 1) {
            ...
            for (let i = 0; i < (arr.length - 1); i++) {
                distance = distance + GetDistance(arr[i].lat, arr[i].lng, arr[1 + 1].lat, arr[i + 1].lng);
