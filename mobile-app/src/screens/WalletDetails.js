  ...
  return (
    <View style={styles.mainView}>
      <Header
        ...
        rightComponent={ auth.info && auth.info.profile && auth.info.profile.usertype =='driver'?<TouchableOpacity onPress={doWithdraw}><Text style={{color:'#fff'}}>{language.withdraw}</Text></TouchableOpacity>:null}
          ...
          <View >
            <View style={{ flexDirection: 'row', justifyContent: "space-around", marginTop: 8 }}>
              <View style={{ height: walletBar - 50, width: '48%', backgroundColor: '#D5D5D5', borderRadius: 8, justifyContent: 'center', flexDirection: 'column' }}>
                ...
                <Text style={{ textAlign: 'center', fontSize: 25, fontWeight: '500', color: '#1CA84F' }}>{settings.symbol}{auth.info && auth.info.profile ? parseFloat(auth.info.profile.walletBalance).toFixed(2) : ''}</Text>
              </View>
