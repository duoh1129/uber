...
export default function AddMoneyScreen(props) {
  ...
  const newData = ({ item, index }) => {
    return (
      <TouchableOpacity style={[styles.boxView, { backgroundColor: item.selected ? colors.GREY.default : '#e6e6e6' }]} onPress={() => { quckAdd(index); }}><Text style={styles.quckMoneyText, { color: item.selected ? '#fff' : '#000' }} >{settings.symbol}{item.amount}</Text></TouchableOpacity>
    )
  }
