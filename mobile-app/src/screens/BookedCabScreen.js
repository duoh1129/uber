...
export default function BookedCabScreen(props) {
    ...
    useEffect(()=>{
        ...
        if(curBooking && curBooking.status == 'ENDED'){
            setTimeout(()=>{
                mapRef.current.fitToCoordinates([{ latitude: curBooking.pickup.lat, longitude: curBooking.pickup.lng }, { latitude: curBooking.drop.lat , longitude: curBooking.drop.lng }], {
                    edgePadding: { top: 40, right: 40, bottom: 40, left: 40 },
                    animated: true,
    ...
    useEffect(() => {
        if (activeBookings && activeBookings.length >= 1) {
            ...
            if (booking.status == 'CANCELLED') {
                if( role == 'rider') props.navigation.navigate('RideList', { booking: booking });
            }
        }
        else {
            ...
            if (role == 'driver') {
                ...
                props.navigation.navigate('RideList',{refresh:true});
            }
        }
    }, [activeBookings]);
            ...
            <View style={styles.mapcontainer}>
                {curBooking ?
                    ...
                    >
                        ...
                        {curBooking.status == 'ARRIVED' || curBooking.status == 'ENDED'?
                            <MapView.Polyline
