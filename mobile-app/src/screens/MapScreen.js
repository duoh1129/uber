...
export default function MapScreen(props) {
    ...
    const _getLocationAsync = async () => {
        ...
        if (status !== 'granted') {
            Alert.alert(language.alert, "Location Permission Issue");
        } else {
            ...
            setTimeout(() => {
                ...
                if (diffMins < 15) {
                    ...
                    dispatch(getEstimate({
                        ...
                        bookingDate: date,
                        platform:'app',
                        pickup: {coords: {lat:tripdata.pickup.lat, lng:tripdata.pickup.lng}},
                        drop:  {coords: {lat:tripdata.drop.lat, lng:tripdata.drop.lng}},
                        carDetails: tripdata.carType,
                    }));
                }
            }, 1000);
                ...
                <View style={{ flex: 1, backgroundColor: "rgba(22,22,22,0.8)", justifyContent: 'center', alignItems: 'center' }}>
                    <View style={{ width: '80%', backgroundColor: "#fffcf3", borderRadius: 10, justifyContent: 'center', alignItems: 'center', flex: 1, maxHeight: 325 }}>
                        <View style={{ marginTop: 0, alignItems: "center" }}>
                            ...
                            <View>
                                <Text style={{ color: "#000", fontSize: 16, marginTop: 12, textAlign: "center" }}>{language.refferal_bonus_messege_text} {settings.code}{bonusAmmount}</Text>
                            </View>
                ...
                <ScrollView horizontal={true} pagingEnabled={true} showsHorizontalScrollIndicator={false}>
                    {activeBookings.map((booking, key) => {
                        ...
                        <Text style={{marginLeft:10,width: 118, color:'red', fontFamily:'Roboto-Bold', fontSize:14}}>{language.active_booking}</Text>
                            <Text style={{marginLeft:10, width: width - 180,marginRight:10,color:'#000'}} numberOfLines={1} ellipsizeMode='tail'>{booking.drop.add}</Text>           
