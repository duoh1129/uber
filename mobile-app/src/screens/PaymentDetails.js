    ...
    if (payDetails.amount >= item.min_order) {
      ...
      if (userAvail != undefined) {
        if (toDay > expDate && userAvail.count == item.promo_usage_limit) {
          ...
          Alert.alert(language.alert,language.promo_limit)
        } else {
          let discounttype = item.promo_discount_type.toUpperCase();
          if (discounttype == 'PERCENTAGE') {
            let discount = payDetails.amount * item.promo_discount_value / 100;
            if (discount > item.max_promo_discount_value) {
              ...
              data.payableAmmount = data.payableAmmount - discount
              setPayDetails(data);
              setPromodalVisible(false);
            } else {
              let data = { ...payDetails };
        ...
        if (toDay > fexpDate) {
          ...
          let discounttype = item.promo_discount_type.toUpperCase();
          if (discounttype == 'PERCENTAGE') {
            var discount = payDetails.amount * item.promo_discount_value / 100;
            if (discount > item.max_promo_discount_value) {
              let discount = item.max_promo_discount_value;
              let data = { ...payDetails };
              data.discount = discount
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollStyle}>
        <View style={{ flex: 1, flexDirection: 'column' }}>
          <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 20, paddingRight: 20, marginBottom: 4 }}>
            <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 22, fontWeight: '500' }}>{language.bill_details}</Text>
            {userdata && userdata.usertype == 'rider' ? payDetails.promo_applied ?
              ...
              <TouchableOpacity
                ...
                <Text style={{ color: '#018509', textAlign: 'left', lineHeight: 45, fontSize: 14, fontWeight: '500' }}>{language.apply_promo}</Text>
              </TouchableOpacity>
              : null}
          </View>
          ...
          {userdata && userdata.usertype == 'driver' ?
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{language.distance}</Text>
              <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{booking ? booking.distance ? parseFloat(booking.distance / 1000).toFixed(2) + ' Kms' : '0.00 Km' : '0.00 Km'}</Text>
            </View>
            : null}
          {userdata && userdata.usertype == 'driver' ?
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{language.total_time}</Text>
              <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{booking ? booking.distance ? parseFloat(booking.total_trip_time / 60).toFixed(1) + ' Mins' : '0.0 Mins' : '0.0 Mins'}</Text>
            </View>
            : null}
          {userdata && userdata.usertype == 'driver' ?
            <View style={{
          ...
          {userdata && userdata.usertype == 'rider' ?
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{language.your_fare}</Text>
              <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{settings.symbol} {parseFloat(payDetails.amount).toFixed(2)}</Text>
            </View>
          ...
          {userdata && userdata.usertype == 'rider' ?
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{language.promo_discount}</Text>
              <Text style={{ color: '#BB1E1E', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>- {settings.symbol} {payDetails ? payDetails.discount ? parseFloat(payDetails.discount).toFixed(2) : '0.00' : '0.00'}</Text>
            </View>
            : null}
          {useWalletCash ?
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: '#000', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>{language.wallet_discount}</Text>
              <Text style={{ color: '#BB1E1E', textAlign: 'left', lineHeight: 45, fontSize: 16 }}>- {settings.symbol} {payDetails ? payDetails.usedWalletMoney ? parseFloat(payDetails.usedWalletMoney).toFixed(2) : '0.00' : '0.00'}</Text>
            </View> : null}
          ...
          {userdata && userdata.usertype == 'rider' ?
            <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 25, paddingRight: 25 }}>
              <Text style={{ color: '#018509', textAlign: 'left', lineHeight: 45, fontSize: 18, fontWeight: '500' }}>{language.payable_ammount}</Text>
              <Text style={{ color: '#018509', textAlign: 'left', lineHeight: 45, fontSize: 18, fontWeight: '500' }}>{settings.symbol} {payDetails.payableAmmount ? parseFloat(payDetails.payableAmmount).toFixed(2) : 0.00}</Text>
            </View>
            : null}
        </View>
